...
import { WithdrawalEntryComponent } from '../withdrawal-entry/withdrawal-entry.component';
...
export class WithdrawalListComponent implements OnInit {
  ...
  displayedColumns: string[] = ['WithdrawalSlipID', 'DateEntry', 'Employee', 'ReferenceNo', 'Amount', 'Status', 'actions'];
    ...
    this.service.getList().subscribe(item => {
      ...
      this.dataSource.filterPredicate = (data, filter) => {
        return this.displayedColumns.some(ele => {
          return ele != 'ReferenceNo' && ele != 'Amount' && ele != 'Status' && ele != 'actions' && data[ele].toLowerCase().indexOf(filter) != -1;
        });
      };
    });
  }
  ...
  onAdd() {
    ...
    const dialogRef = this.dialog.open(WithdrawalEntryComponent, dialogconfig);
  ...
  onApprove(id) {
    this.comfirmDialogService.openConfirmDialog('Are you sure to approve this record?')
      .afterClosed().subscribe(res => {
        if (res) {
          this.service.updateStatus(parseInt(id), '/WS/updateStatus/Approved/').subscribe(
            (res: any) => {
  ...
  onCancel(id) {
    this.comfirmDialogService.openConfirmDialog('Are you sure to cancel this record?')
      .afterClosed().subscribe(res => {
        if (res) {
          this.service.updateStatus(parseInt(id), '/WS/updateStatus/Cancelled/').subscribe(
            (res: any) => {
  ...
  onUnpost(id) {
    this.comfirmDialogService.openConfirmDialog('Are you sure to un-post this record?')
      .afterClosed().subscribe(res => {
        if (res) {
          this.service.updateStatus(parseInt(id), '/WS/updateStatus/Unpost/').subscribe(
            (res: any) => {
