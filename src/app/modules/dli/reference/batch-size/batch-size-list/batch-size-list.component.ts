...
import { BatchSizeEntryComponent } from '../batch-size-entry/batch-size-entry.component';
...
export class BatchSizeComponent implements OnInit {
  ...
  constructor(private service: BatchSizeService,
    private dialog: MatDialog,
    private notificationService: NotificationService,
    private comfirmDialogService: ConfirmDialogService) { }

  dataSource: MatTableDataSource<any>;
  displayedColumns: string[] = ['BatchSizeID', 'BatchSize', 'actions'];
  ...
  onAdd() {
    ...
    const dialogRef = this.dialog.open(BatchSizeEntryComponent, dialogconfig);
  ...
  onEdit(row: number) {
    this.service.getByID(row).subscribe(data => {
      this.service.formBatchSize.patchValue({
