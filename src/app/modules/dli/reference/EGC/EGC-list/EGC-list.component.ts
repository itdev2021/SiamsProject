...
import { EGCEntryComponent } from '../EGC-entry/EGC-entry.component';
...
export class EGCComponent implements OnInit {
  ...
  constructor(private service: EGCService,
    private dialog: MatDialog,
    private notificationService: NotificationService,
    private comfirmDialogService: ConfirmDialogService) { }

  dataSource: MatTableDataSource<any>;
  displayedColumns: string[] = ['EGCID', 'EGC', 'actions'];
  ...
  onAdd() {
    ...
    const dialogRef = this.dialog.open(EGCEntryComponent, dialogconfig);
  ...
  onEdit(row: number) {
    this.service.getByID(row).subscribe(data => {
      this.service.formEGC.patchValue({
        EGCID: data.EGCID,
        EGC: data.EGC
      });
