...
import { UOMEntryComponent } from '../uom-entry/uom-entry.component';
...
export class UOMComponent implements OnInit {
  ...
  displayedColumns: string[] = ['UOMID', 'Code','UOMDescription', 'actions'];
  ...
  onAdd() {
    ...
    const dialogRef = this.dialog.open(UOMEntryComponent, dialogconfig);
  ...
  onEdit(row: number) {
    this.service.getByID(row).subscribe(data => {
      this.service.formUOM.patchValue({
        UOMID: data.UOMID,
        Code: data.Code,
        UOMDescription: data.UOMDescription
      });
