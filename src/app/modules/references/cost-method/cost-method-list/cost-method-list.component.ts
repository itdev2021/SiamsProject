...
import { CostMethodEntryComponent } from '../cost-method-entry/cost-method-entry.component';



@Component({
  selector: 'app-cost-method-list',
  templateUrl: './cost-method-list.component.html',
  styles: []
})
export class CostMethodComponent implements OnInit {

  constructor(private service: CostMethodService,
    private dialog: MatDialog,
    private notificationService: NotificationService,
    private comfirmDialogService: ConfirmDialogService) { }

    dataSource: MatTableDataSource<any>;
    displayedColumns: string[] = ['CostMethodID','CostMethod','actions'];
  ...
  onAdd(){
    ...
    const dialogRef = this.dialog.open(CostMethodEntryComponent, dialogconfig);
  ...
  onEdit(row: number){
    this.service.getByID(row).subscribe(data => {
      this.service.formCostMethod.patchValue({
