...
import { CashDisbursementEntryComponent } from '../cash-disbursement-entry/cash-disbursement-entry.component';

@Component({
  selector: 'app-cash-disbursement-list',
  templateUrl: './cash-disbursement-list.component.html',
  styles: []
})
export class CashDisbursementListComponent implements OnInit {

  dataSource: MatTableDataSource<any>;
  displayedColumns: string[] = ['DisbursementID', 'DisbursementDate', 'CheckNo', 'Supplier', 'TotalPayment', 'StatusCode', 'actions'];
  ...
  onAdd() {
    ...
    const dialogRef = this.dialog.open(CashDisbursementEntryComponent, dialogconfig);
  ...
  onApprove(id) {
    ...
    this.comfirmDialogService.openConfirmDialog('Are you sure to approve this record?')
      .afterClosed().subscribe(res => {
        if (res) {
          this.service.updateStatus(parseInt(id), '/Disbursement/updateStatus/Approved/', 1).subscribe(
            (res: any) => {
  ...
  onCancel(id) {
    ...
    this.comfirmDialogService.openConfirmDialog('Are you sure to cancel this record?')
      .afterClosed().subscribe(res => {
        if (res) {
          this.service.updateStatus(parseInt(id), '/Disbursement/updateStatus/Cancelled/', -1).subscribe(
            (res: any) => {
  ...
  onUnpost(id) {
    this.comfirmDialogService.openConfirmDialog('Are you sure to un-post this record?')
      .afterClosed().subscribe(res => {
        if (res) {
          this.service.updateStatus(parseInt(id), '/Disbursement/updateStatus/Unpost/', 0).subscribe(
            (res: any) => {
