...
import { IDSEntryComponent } from '../ids-entry/ids-entry.component';
...
export class IDSToListComponent implements OnInit {

  dataSource: MatTableDataSource<any>;
  displayedColumns: string[] = ['IDSID', 'IDSDate', 'Reference', 'PayTo', 'Remarks', 'Debit', 'Credit', 'StatusCode', 'actions'];
      ...
      this.service.getList().subscribe(item => {
        ...
        this.dataSource.filterPredicate = (data, filter) => {
          return this.displayedColumns.some(ele => {
            return ele != 'IDSDate' && ele != 'actions' && data[ele].toLowerCase().indexOf(filter) != -1;
          });
        };
      });
    });
  ...
  onAdd() {
    ...
    const dialogRef = this.dialog.open(IDSEntryComponent, dialogconfig);
  ...
  onApprove(id) {
    ...
    this.comfirmDialogService.openConfirmDialog('Are you sure to approve this record?')
      .afterClosed().subscribe(res => {
        if (res) {
          this.service.updateStatus(parseInt(id), '/IDS/updateStatus/Approved/', 1).subscribe(
            (res: any) => {
  ...
  onCancel(id) {
    ...
    this.comfirmDialogService.openConfirmDialog('Are you sure to cancel this record?')
      .afterClosed().subscribe(res => {
        if (res) {
          this.service.updateStatus(parseInt(id), '/IDS/updateStatus/Cancelled/', -1).subscribe(
            (res: any) => {
  ...
  onUnpost(id) {
    this.comfirmDialogService.openConfirmDialog('Are you sure to un-post this record?')
      .afterClosed().subscribe(res => {
        if (res) {
          this.service.updateStatus(parseInt(id), '/IDS/updateStatus/Unpost/', 0).subscribe(
            (res: any) => {
