...
import { PurchaseReturnEntryComponent } from '../purchase-return-entry/purchase-return-entry.component';
...
export class PurchaseReturnListComponent implements OnInit {
  ...
  displayedColumns: string[] = ['SupplierReturnsID', 'SupplierReturnsDate', 'SupplierReturnsNumber', 'ReceivingReceiptReferenceNo', 'Supplier', 'Net', 'StatusCode', 'actions'];
      ...
      this.service.getList().subscribe(item => {
        ...
        this.dataSource.filterPredicate = (data, filter) => {
          return this.displayedColumns.some(ele => {
            return ele != 'SupplierReturnsDate' && ele != 'Net' && ele != 'actions' && data[ele].toLowerCase().indexOf(filter) != -1;
          });
        };
      });
    });
  ...
  onAdd() {
    ...
    const dialogRef = this.dialog.open(PurchaseReturnEntryComponent, dialogconfig);
  ...
  onApprove(id) {
    this.comfirmDialogService.openConfirmDialog('Are you sure to approve this record?')
      .afterClosed().subscribe(res => {
        if (res) {
          this.service.updateStatus(parseInt(id), '/SupplierReturn/updateStatus/Approved/', 1).subscribe(
            (res: any) => {
  ...
  onCancel(id) {
    this.comfirmDialogService.openConfirmDialog('Are you sure to cancel this record?')
      .afterClosed().subscribe(res => {
        if (res) {
          this.service.updateStatus(parseInt(id), '/SupplierReturn/updateStatus/Cancelled/', -1).subscribe(
            (res: any) => {
  ...
  onUnpost(id) {
    this.comfirmDialogService.openConfirmDialog('Are you sure to un-post this record?')
      .afterClosed().subscribe(res => {
        if (res) {
          this.service.updateStatus(parseInt(id), '/SupplierReturn/updateStatus/Unpost/', 0).subscribe(
            (res: any) => {
